...
namespace XP.TableModel.Test
{
    ...
    /// 测试表格
    /// </summary>
    public class m2i7 : MonoBehaviour
    {   
        private List<List<string>> _TableData; // 用于存储从文本文件加载的原始数据
        private List<variaData> _TestDataList; // 用于将原始数据转换为 TestData 对象的列表
...
private void _TestBindArr2(List<List<string>> data)
{
    ...
    // 创建 variaData 列表
    List<variaData> _TestDatas = new List<variaData>();

    // 循环创建 TestData 对象并添加到列表
    foreach (var row in data)
    {
        _TestDatas.Add(new variaData { name = row[0], number =row[1], comment = row[2] });
...
private void _DefaultArr()
{
    ...
    List<variaData> _TestDatas = new List<variaData>
    {
        new variaData { name = "HMC_OUT", number = "10",  comment = "(stp pulse)" },
        new variaData { name = "HMC_OUT_0", number = "10", comment = "(stp pulse)" },
        new variaData { name = "HMC_OUT_0", number = "10", comment = "(stp pulse)" },
        ...
        foreach (string line in lines)
        {
            // Check if the line starts with "$"
                ...
                if (keyValue.Length == 4)
                {
                    ...
                    _TableData.Add(new List<string> { column1, column2, column3 });
                }
