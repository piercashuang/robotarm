...
namespace XP.TableModel.Test
{
    ...
    /// 测试表格
    /// </summary>
    public class m2i5 : MonoBehaviour
    {   
        private List<List<string>> _TableData; // 用于存储从文本文件加载的原始数据
        private List<simulateData> _TestDataList; // 用于将原始数据转换为 TestData 对象的列表
...
private void _TestBindArr2(List<List<string>> data)
{
    ...
    // 创建 simulateData 列表
    List<simulateData> _TestDatas = new List<simulateData>();

    // 循环创建 TestData 对象并添加到列表
    foreach (var row in data)
    {
        _TestDatas.Add(new simulateData { name = row[0], number =row[1], status =row[2],comment = row[3] });
...
private void _DefaultArr()
{
    ...
    List<simulateData> _TestDatas = new List<simulateData>
    {
        new simulateData { name = "HMC_OUT", number = "10", status = "(stp pulse)", comment = "(stp pulse)" },
        new simulateData { name = "HMC_OUT_0", number = "10", status = "(delay)", comment = "(stp pulse)" },
